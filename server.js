    ...
    case 'set_autorize' :  /* Команда авторизации в Приложении*/
        ...
        Firebird.attach(options, function (err, db) {
            if (err)
                throw err;
            // db = DATABASE
            db.query("select w.Working,w.worker_id,w.surname||w.firstname worker_name,w.midlename,w.Ibname from Worker w where w.Ibname = '" + login + "' and w.Working = 1 and w.IN_REQUEST=1", function (err, result) {
...
// перебор элементов массива
            request_list.forEach(function (item, i, arr) {
				...
				console.log(login);
				 db.query("UPDATE REQUEST_GIVED  SET sended=1 , date_sended=CURRENT_TIMESTAMP WHERE req_id=? and IBNAME=?", [item.req_id,login], function (err, result) {
